...
Queue (antrian) merupakan struktur data linear dasar yang menerapkan prinsip FIFO (First-In, First-Out), yaitu elemen yang masuk lebih dahulu melalui operasi enqueue pada bagian rear/tail akan menjadi elemen pertama yang keluar melalui operasi dequeue pada bagian front/head. Struktur data ini dapat diimplementasikan menggunakan array, linked list, atau circular array, serta berperan penting dalam berbagai aplikasi, seperti penjadwalan proses pada sistem operasi dan algoritma Breadth-First Search (BFS).
...
Program ini merealisasikan struktur data Queue (antrean) statis dengan memanfaatkan array berukuran tetap (MAX = 5) serta dua penunjuk, yaitu head dan tail. Antrean diawali dalam keadaan kosong, yang ditandai dengan nilai head dan tail sama dengan −1. Program ini menyediakan operasi dasar berupa enqueue untuk menambahkan elemen pada bagian belakang antrean, dequeue untuk menghapus elemen dari bagian depan dengan melakukan pergeseran elemen ke kiri, isEmpty dan isFull untuk memeriksa kondisi antrean, serta printQueue untuk menampilkan isi antrean. Pada fungsi main, ditunjukkan proses penambahan dan penghapusan elemen secara berurutan guna memperlihatkan mekanisme kerja antrean berdasarkan prinsip FIFO (First In First Out).
...
Pada alternatif 1, queue menggunakan array dengan head tetap di indeks 0 dan tail bergerak. Penambahan elemen dilakukan di tail, sedangkan penghapusan dilakukan dengan menggeser seluruh elemen ke kiri, sehingga sederhana tetapi kurang efisien karena memerlukan pergeseran data saat dequeue.
